...
class SASRec(nn.Module):
    ...
    def forward(self, src):
        ...
        item_embs = self.embed(torch.LongTensor(torch.ones(src.size()).type(torch.LongTensor)).to(self.device))

        logits = self.item_embs.matmul(final_feat.unsqueeze(-1)).squeeze(-1)
