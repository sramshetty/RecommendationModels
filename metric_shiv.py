...
def get_recall_item(indices, targets, mask):
...
def get_mrr_item(indices, targets, mask):
    targets = targets.expand_as(indices)
    hits = (targets == indices)
    hits *= mask[:20]
...
def evaluate_item(indices, targets, mask, k=20, debug=False):
    _, indices = torch.topk(indices, k, -1)
    
    indices = indices.cpu()
    targets = targets.cpu()

    recall = get_recall_item(indices, targets, mask)
    mrr = get_mrr_item(indices, targets, mask)
